...
public class BCParticleUtil {
    public static void spawnParticleForSettings(ClientPlayerEntity player, AttackHand hand, List<ParticleSettings> settingsList, float weaponRange,
        ...
        for (ParticleSettings settings : settingsList) {
            ...
            if(BetterCombatParticleMods.config.triggerOwnParticle){
    ...
    public static void sendPacketParticleData(ClientPlayerEntity player, AttackHand hand, List<ParticleSettings> settingsList, float weaponRange,
                                              boolean light, String colorHex, String colorHexSec) {
        for (ParticleSettings settings : settingsList) {
            boolean isOffhand = hand.isOffHand();
            float offhandRoll = isOffhand ? 180 : 0;
            ...
            NetworkHandler.sendParticlePacket(animationName,x, y, z, xStab, yStab, zStab,
                    player.getPitch(), player.getYaw(), weaponRange, isOffhand,
                    light, colorHex, colorHexSec
            );
        }
    }
}
