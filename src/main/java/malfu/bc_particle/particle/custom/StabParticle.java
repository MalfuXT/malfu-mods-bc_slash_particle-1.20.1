...
public class StabParticle extends SlashParticle180 {


    StabParticle(ClientWorld world, double x, double y, double z, float scale, float pitch, float yaw, float localYaw, float roll, boolean light, String colorHex, SpriteProvider spriteProvider) {
        super(world, x, y, z, scale, pitch, yaw, localYaw, roll, light, colorHex, spriteProvider);
        this.scale = (float) Math.min((scale*1.7/2.5), 2.75);
    }
    ...
    public static class Provider implements ParticleFactory<SlashParticleEffect> {
        ...
        public Particle createParticle(SlashParticleEffect settings, ClientWorld clientWorld, double d, double e, double f, double g, double h, double i) {
            return new StabParticle(clientWorld, d, e, f, settings.getScale(), settings.getPitch(), settings.getYaw(), settings.getLocalYaw() ,
